...
sim_AR1_xts <- function(J, T, resp_coefs = NULL, 
  ...
  if (is.null(resp_coefs)) {
    ...
    resp_coef_vals <- sample(rnorm(N, 0, 3), 4)
    names(resp_coef_vals) <- c("treat", "beta", "gamma", "delta")
  }
  # If user covariate values of length 5 are provided use those
  else if (!is.null(resp_coefs) & length(resp_coefs) == 4) {
  ...
  # For y[1[j]] ~ half-normal(mu[1[j]], 3)
  ...
  treatment_grid <- resp_grid %>% 
    ...
    mutate(
      ...
      treat = treat_coef_vals["intercept"] + (time/1000) + 
        (treat_coef_vals["beta"] * beta) + (treat_coef_vals["gamma"] * gamma) + 
        (treat_coef_vals["delta"] * delta) + rnorm(n(), 0, 0.25),
      # Generate a binary indicator after eight treatment status change
      post_treat = ifelse(rleid(rbinom(n(), 1, prob = rescale(treat, to = c(0, 0.25)))) >= 8, 1, 0),
      # Induce dependence of the response on the treatment
      y_treated = Y + lag(Y, n = 1L) + (resp_coef_vals["beta"] * beta) + 
        (resp_coef_vals["gamma"] * gamma) + (resp_coef_vals["delta"] * delta) + 
        (resp_coef_vals["treat"] * post_treat) + rnorm(n(), 0, 0.5),
      # Recode Time
      time = time - 2,
      # Generate lags of the covariates and treatment
      across(
        Y:y_treated,
        ~ lag(.x, 1L),
        .names = "{.col}_lag"
...
treat_trues <- rnorm(4, 0, 2)*rgamma(4, 1, 1)
...
sim_data <- sim_AR1_xts(
  ...
  T = 102, 
  seed = 12345,
  resp_coefs = c(6.8563, 3.1879, -2.4487, 5.4057),
  treat_coefs = treat_trues
  )

# Check distribution of the treatment
xtabs(~ country + post_treat, sim_data$data)
...
sim_data$data %>% 
  ...
  labs(
    x = "Time",
    y = "Response",
    title = "Simulated AR(1) Panel Data",
    color = "Post-Intervention"
  ) +
  # Set plot theme
